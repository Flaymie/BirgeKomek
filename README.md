...
-   **Проблема:** Как обеспечить мгновенную связь между пользователями и отслеживать их онлайн-статус?
    -   **Решение:** Использована библиотека `Socket.IO` в связке с **Redis**. Для статусов реализован механизм "heartbeat": клиент периодически отправляет серверу сигнал "я онлайн", а сервер записывает ID активного пользователя в быструю in-memory базу данных Redis с коротким временем жизни. При разрыве соединения (`disconnect`) или по истечении времени ключ автоматически удаляется. Это позволило создать надежный и производительный механизм отслеживания онлайн-статуса без лишней нагрузки на основную базу данных (MongoDB).
