...
- **Сброс пароля администратором:** Добавлен эндпоинт и функционал для сброса пароля пользователя администратором без необходимости подтверждения по почте.
- **Режим "только для чтения" без Telegram:** Добавлено ограничение функционала для пользователей без привязанного Telegram-аккаунта. Теперь для создания запросов, отправки сообщений и откликов требуется привязка к Telegram.
