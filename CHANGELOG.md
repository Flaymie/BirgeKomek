...
- **Интеграция с Redis**
  - Кэширование данных для ускорения ответов
...
- **Отображение модальных окон:** Исправлена проблема с z-index в универсальном компоненте модального окна, из-за которой содержимое модалки могло отображаться позади слоя с затемнением и размытием.
...
### 🚀 Новое
- **Продвинутая система защиты входа:**
  - При входе с нового, не доверенного IP-адреса, система теперь требует подтверждение через Telegram.
  - Пользователь получает уникальный код в боте, который нужно ввести на сайте.
  - При 3-х неверных попытках ввода кода IP-адрес блокируется на 24 часа для предотвращения брутфорс-атак.
- **Command Palette (Палитра команд):**
  - Добавлена палитра команд (вызывается по `Ctrl+J` или `Cmd+J`), предоставляющая быстрый доступ к основным действиям: навигация, создание заявки, просмотр профиля и т.д.
- **Интеграция с Cloudinary:**
  - Все загружаемые файлы (аватары, вложения в чат) теперь хранятся в облачном хранилище Cloudinary, что снимает нагрузку с сервера и обеспечивает быстрое и надежное хранение.

### 🔒 Безопасность и Защита
- **Защита от взлома:** Для критически важных действий (например, удаление аккаунта, модераторские действия) добавлена дополнительная защита от брутфорс-атак(дается 3 попытки на ввод кода, после чего аккаунт блокируется на 7 дней и отправляется уведомление в Telegram аккаунт юзера).
- **Обфускация и минификация кода:** Для продакшн-сборки фронтенда теперь применяются обфускация и минификация, что значительно затрудняет анализ кода в DevTools и повышает безопасность.
- **Улучшенная система определения IP:** Исправлен баг, из-за которого IP-адрес пользователя мог определяться некорректно. Теперь система более точно определяет реальный IP, игнорируя локальные и прокси-адреса.
- **Улучшенный скоринг при регистрации:** Внесены корректировки в `scoringService.js` для более точной оценки подозрительности новых регистраций.

### 🐞 Исправления
- **Рейтинг:** Полностью исправлена логика подсчета среднего рейтинга хелпера. Теперь рейтинг корректно пересчитывается после каждого нового отзыва. Для пользователей без выполненных заказов отображается рейтинг по умолчанию, а не "Н/Д".
- **Система репортов:** Устранен баг, позволявший нескольким администраторам одновременно работать с одной и той же жалобой. Теперь, как только один администратор берет репорт в работу, он блокируется для других.
- **Регистрация через Telegram:** Исправлена ошибка, из-за которой при регистрации через Telegram-бота не сохранялся IP-адрес пользователя после первого входа на сайт.
- **Чат после завершения заявки:** Устранен критический баг, позволявший хелперу продолжать писать в чат даже после того, как заявка была завершена. Теперь чат корректно блокируется для всех участников.
- **Модальное окно привязки Telegram:** Исправлена ошибка, из-за которой периодически появлялось назойливое модальное окно с требованием привязать Telegram, даже если он уже был привязан.
- **Обработка истекших токенов:** Улучшена обработка истекших JWT-токенов. Теперь пользователь получает более понятное сообщение и корректно перенаправляется на страницу входа.
- **Индексация в боте:** Исправлены ошибки, связанные с индексацией и обработкой команд в Telegram-боте.

### 🎨 Улучшения и прочие исправления
- Множество мелких исправлений в верстке, текстах и логике работы интерфейса.