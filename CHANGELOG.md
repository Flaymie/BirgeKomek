...
### ✨ Добавлено
- **Двухфакторное подтверждение модерации:** Реализовано подтверждение действия "Забанить пользователя" через личные сообщения в Telegram. Модератору приходит уведомление с кнопками для подтверждения или отмены действия. Это повышает безопасность и предотвращает случайные баны.

### ♻️ Изменено
- Обновлен `README.md` с описанием новой переменной окружения `INTERNAL_BOT_KEY`.

### 🐞 Исправлено
- Исправлена ошибка, из-за которой аватар пользователя не обновлялся в реальном времени после загрузки нового.
- Устранена проблема с некорректным отображением длинных имен пользователей в профиле.