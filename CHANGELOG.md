...
- **API для счетчика:** На бэкенде добавлен эндпоинт (`/api/notifications/unread/count`), который предоставляет точное количество непрочитанных уведомлений.

### 🐞 Исправлено
- **Авто-скролл в чате:** Устранена критическая ошибка, из-за которой страница чата не прокручивалась автоматически в самый низ к последним сообщениям при загрузке.
