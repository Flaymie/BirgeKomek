...
- Добавлены бейджи ролей пользователей на странице всех отзывов для наглядности.
- Реализован компонент `CityAutocomplete` с интеграцией API DaData для динамического поиска городов Казахстана, заменяющий статический список.
- Создан кастомный хук `useDebounce` для оптимизации запросов к API при вводе текста.
- Добавлен защищенный эндпоинт на бэкенде для предоставления API-ключа DaData фронтенду.
...
- Устранена ошибка 401 Unauthorized при запросе ключа DaData, убрав избыточную проверку авторизации.
- Исправлено некорректное форматирование названий городов (например, "Алматы, г Алматы") в компоненте CityAutocomplete.
