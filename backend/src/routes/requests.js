   ...
   *     responses:
    ...
    router.put('/:id/status', protect, [
        ...
        try {
            ...
            // --- ПРОВЕРКИ ДОСТУПА И ПЕРЕХОДОВ ---

            // 1. Нельзя менять статус чужой заявки (кроме админа/модератора)
            // (Хотя middleware 'protect' проверяет авторизацию, нужно проверить права на конкретную заявку)
            // В данном случае, мы разрешаем автору и хелперу менять статусы, но с ограничениями ниже.

            if (newStatus === 'open') {
                if (oldStatus === 'draft') {
                    // ПУБЛИКАЦИЯ ЧЕРНОВИКА (Только автор)
                    if (!isAuthor) {
                    ...
                    // ПОПЫТКА "ПЕРЕОТКРЫТЬ" (RE-OPEN) через обычный роут
                    // Разрешаем только если статус был 'assigned' (сброс хелпера) или 'in_progress'. 
                    // Если статус 'completed', 'closed', 'cancelled' - ЗАПРЕЩАЕМ через этот роут.
                    // Блокируем "воскрешение" закрытых заявок.
                    return res.status(400).json({ msg: 'Нельзя перевести заявку в статус Open напрямую. Используйте функцию "Переоткрыть" (Reopen), если заявка была в работе.' });
                }
            }

            else if (newStatus === 'completed') {
                // ЗАВЕРШЕНИЕ
                if (!isAuthor && !(request.helper && request.helper.toString() === userId)) {
                    return res.status(403).json({ msg: 'Только автор или назначенный помощник могут завершить заявку.' });
                }
            }
